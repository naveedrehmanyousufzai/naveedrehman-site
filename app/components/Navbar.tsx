...
export default function Navbar() {
  ...
  return (
    <nav className="fixed w-full z-50 bg-white/95 backdrop-blur-md border-b-2 border-[#D4AF37]/30 text-black transition-all shadow-sm">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 h-20 flex items-center justify-between">
        
        {/* BRAND / LOGO (Original Colors) */}
      ...
      <div 
        className={`md:hidden absolute top-20 left-0 w-full bg-white border-b-2 border-[#D4AF37]/50 overflow-hidden transition-all duration-300 ease-in-out shadow-lg ${
          isMobileMenuOpen ? "max-h-64 opacity-100 py-4" : "max-h-0 opacity-0 py-0"
        }`}
      >
        <div className="flex flex-col px-6 gap-4 text-sm font-bold uppercase tracking-widest text-gray-800">
          <Link 
            href="/" 
            onClick={toggleMenu} 
