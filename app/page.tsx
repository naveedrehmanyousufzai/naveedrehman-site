...
export default async function HomePage() {
  // Fetch Sanity Posts (Only for News and Pictures now)
  ...
  const pictures = posts.filter((post: any) => post.postType === 'photo' || (!post.postType && !post.externalLink && post.imageUrl)).slice(0, 6);

  // Fetch LIVE YouTube Videos directly from Google
  const liveYouTubeVideos = await getLatestYouTubeVideos(3);
      ...
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="relative z-0 flex flex-col md:flex-row items-start md:items-center justify-between gap-12 mb-24 border-b-2 border-[#D4AF37]/20 pb-16 pt-8">
          <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
            <img src="/hero.png" alt="Squash Court Background" className="w-full h-full object-cover opacity-100" />
          ...
          <div className="max-w-3xl relative z-10">
            <AnimatedSection delay={0.1}>
              <span className="text-[#D4AF37] font-black uppercase tracking-widest text-sm block mb-4">Official Platform</span>
              ...
              <div className="flex flex-wrap gap-4">
                <Link href="/tournaments" className="bg-[#D4AF37] hover:bg-black text-white font-black uppercase tracking-widest px-8 py-4 rounded transition-colors text-sm shadow-md">Career Archive</Link>
                <Link href="/about" className="bg-white hover:bg-gray-50 border-2 border-[#D4AF37] text-black font-bold uppercase tracking-widest px-8 py-4 rounded transition-colors text-sm shadow-sm">Athlete Profile</Link>
              </div>
            </AnimatedSection>
          </div>
        </div>

        {/* --- SECTION 1: LIVE YOUTUBE VIDEOS --- */}
        <AnimatedSection delay={0.2}>
          <div className="mb-20">
            <div className="flex justify-between items-end mb-8 border-b-2 border-[#D4AF37]/20 pb-4">
              <h2 className="text-2xl font-black uppercase tracking-tight text-black">Latest Videos</h2>
            </div>
            
            {liveYouTubeVideos.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {liveYouTubeVideos.map((video: any) => (
                  <a
                    key={video._id}
                  ...
                  >
                    <div className="relative aspect-video overflow-hidden border-b-2 border-[#D4AF37]/20 bg-gray-100">
                      <img
                        src={video.imageUrl || '/placeholder.jpg'}
                        alt={video.title}
        ...
        <AnimatedSection delay={0.2}>
          <div className="mb-20">
            <div className="flex justify-between items-end mb-8 border-b-2 border-[#D4AF37]/20 pb-4">
              <h2 className="text-2xl font-black uppercase tracking-tight text-black">News & Articles</h2>
            </div>
            ...
            {articles.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="p-6">
                        ...
                        <h3 className="text-lg font-bold text-black group-hover:text-[#D4AF37] transition-colors line-clamp-2 mb-3">{post.title}</h3>
                        <div className="text-xs font-black uppercase tracking-widest text-gray-500 group-hover:text-[#D4AF37] transition-colors flex items-center gap-2">
               ...
               <div className="bg-gray-50 p-8 rounded border-2 border-dashed border-[#D4AF37]/30 text-center text-gray-500 font-medium">No articles published yet.</div>
            )}
          </div>
        </AnimatedSection>
        ...
        <AnimatedSection delay={0.2}>
          <div>
            <div className="flex justify-between items-end mb-8 border-b-2 border-[#D4AF37]/20 pb-4">
              <h2 className="text-2xl font-black uppercase tracking-tight text-black">Latest Pictures</h2>
            </div>
            ...
            {pictures.length > 0 ? (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
               ...
               <div className="bg-gray-50 p-8 rounded border-2 border-dashed border-[#D4AF37]/30 text-center text-gray-500 font-medium">No pictures uploaded yet.</div>
            )}
          </div>
        </AnimatedSection>
